...
export async function sendUserOperation<T extends readonly unknown[]>(
  ...
  const signature = await wallet.account.sign({ hash: createOpResponse.data.unsigned_payload as `0x${string}` })
  ...
  const returnValue: SendUserOperationReturnType = {
    ...
    wait: async () => {
      ...
      return wait(
        reload,
        (op) => op.status === UserOperationStatusEnum.Complete || op.status === UserOperationStatusEnum.Failed,
