...
;(async function(){
  ...
  console.log('\n\n------------------------------------------------------------------------------------------------------------------');
  console.log(`We're going to transfer ${TRANSFER_AMOUNT} ${ASSET} ${TRANSFER_NUMBER} times from myWallet to anotherWallet...`);
  console.log('------------------------------------------------------------------------------------------------------------------');
  ...
  for (let i = 1; i <= TRANSFER_NUMBER; i++) {
    ...
    try {
      ...
      console.log(`Some error happened (this doesn't necessarily mean transfer failed) :`, e.toString());
    }
    console.log('------------------------------------------------------------------------------------------------------------------');
  }
  
  console.log('Funds available on myWallet after transfers:', (await myWallet.listBalances()).toString());
  console.log('Funds available on anotherWallet after transfer (previously no balance):', (await anotherWallet.listBalances()).toString());


  console.log('\n\n------------------------------------------------------------------------------------------------------------------');
  console.log(`We're going to transfer ${TRANSFER_AMOUNT} ${ASSET} back from anotherWallet to myWallet...`);
  console.log('------------------------------------------------------------------------------------------------------------------');
  console.log('Please press Enter to continue...');
  await waitForEnter();
  ...
  console.log('------------------------------------------------------------------------------------------------------------------');
  console.log('\nFunds available on myWallet after transfers:', (await myWallet.listBalances()).toString());
  console.log(`Funds available on anotherWallet after transfer (previously ${(TRANSFER_AMOUNT*TRANSFER_NUMBER).toFixed(4)}):`, (await anotherWallet.listBalances()).toString());
  console.log('\n------------------------------------------------------------------------------------------------------------------');
  ...
  console.log('More details on what is available on transaction object here: https://coinbase.github.io/coinbase-sdk-nodejs/classes/coinbase_transaction.Transaction.html');
  
  const firstTransactionContent = transactions[0].content();
  console.log('\ntransactions[0].content() block_timestamp:', firstTransactionContent.block_timestamp);
  console.log('More details on what is available on transaction.content() here: https://coinbase.github.io/coinbase-sdk-nodejs/interfaces/client_api.EthereumTransaction.html');
